...
## 〇-b、⚠️ 底线项（已闭环：内容+本地历史 0 残留；远端旧对象按裁决②重建仓库处理）
...
🚧 远端对象（GitHub 侧）· 裁决：② 删仓库 + 同名重建
- 事实：GitHub **不立即回收 force-push 后的旧对象** —— 实测历代旧提交与含昵称/含 token 的旧 blob 仍可
  **按 SHA 取回完整内容**（不可浏览、需已知 SHA；仓库 events 流实测 0 条）。本地已 0 残留，远端对象清不掉。
- 裁决（2026-09-14）：走 ② —— 删仓库 + 同名重建 + 重推干净提交（本仓 fork/star/watcher/issue 全 0，代价近零；
  新仓库对象库为空 ⇒ 旧 SHA 一律 404）。备选：① 发 GitHub Support 请求清除；③ 接受现状。
- 验收：重建后旧 SHA 的 blob/commit API 返回 **404**；新仓 HEAD 全树复扫全 0。
```
