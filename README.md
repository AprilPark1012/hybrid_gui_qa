...
**一句话**：把「埋点只顶层有」从口头约定变成**可执行判据**，并按这个口径改造 demo、补齐
「**没有埋点也能定位**」的能力。演示站点撤掉 **115 处**控件级埋点（只留 **27 个顶层容器**）。

**为什么**：框架要对着**生产口径**校验，口径不一致时宁可报错，也不假装通过 ——
否则可用性 / 正确性 / 健壮性都是空谈。

**判据（规则 ⑤⑥，`tests/testid_policy.py`）**
- 交互控件（`input/select/textarea/button/a/label/option`）**无论层级**一律不许有 `testid`
- 弹层**内部**（非容器本身）一律不许
- **只有顶层容器**可保留埋点

**定位侧新增（零 testid 下依然唯一定位）**
- **字段步**：工具栏「…」这类无文本无埋点的入口，用同容器内相邻字段的语义定位并**下钻**
- **`title` 消歧**：同名控件/裸图标按 `title` 或邻近文本区分；有歧义**如实失败**（禁止按顺序猜）
- **元素身份** = 容器锚点 + 相对路径 + `title`/邻近文本

**验证工程化（本版重点，全部是踩坑换来的）**
- 二类单脚本预算 **600s → 1200s**（`--timeout-s`；长用例此前是被预算杀掉的，不是断言失败）；
  新增 `--jobs N` 并发（默认 1 与原行为一致）——**浏览器脚本必须过内存闸**（≥550MB × 已跑浏览器数）、
  **独占屏障**（会重写共享产物的脚本独占）
- **产物质检**：`generate` 生成后**自洽闸**（引用不存在的数据集 ⇒ 拒绝落盘）+ 陈旧数据集自愈；
  二类 runner **脚本之间**质检 ⇒ 弄脏产物的脚本**当场点名**；每脚本**完整日志落盘** `log/verify_logs/`

**修掉的坑**：产物「悬空引用」窗口（临时用例被归档后产物没复原 ⇒ 留死引用 ⇒ 后续脚本全红，
且报 `FileNotFoundError` + pytest 退出码 **2**，**看着像环境/偶发**）· 录像体检工具两个真 bug ·
负向判据不再「永远 SKIP」· 并发写竞争假红 · UTF-8 铁律违例

**证据**：一类 **421 passed / 0 red** · 二类 **16/16 全绿** · 端到端 **19/0** · `generate` **35/35 · 0 未映射**

## 2026-09-22（第二次）变更要点 —— 本地验收环境适配（一轮 19 红归零 · 未发版）

**背景**：现场（Windows + 交付包解压目录）跑 `python -m pytest tests/ -q` 得 **19 failed / 258 passed**。
逐条定位后：**19 红 = 4 类真 bug + 1 类现场状态**（其中 9 条「生成物缺接线」是那两个产物文件**为 0 字节**
所致 —— 包内产物经核对完好，不是框架问题）。

| # | 根因（都有复现证据） | 修法 |
|---|---|---|
| ① | 生成物**为空 / 缺失**时，9 条契约判据各报一句「缺接线」/「没有 _goto」，完全看不出真因 | 新增 `tests/artifacts.py`：产物不存在或 0 字节 ⇒ 第一句说清 + 给出下一步（`cli generate` / 重新解压） |
