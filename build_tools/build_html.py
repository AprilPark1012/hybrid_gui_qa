...
CHANGELOG = [
    dict(
        ...
        summary="把「一个 <code>test_cases.py</code> 装全部用例」这件事改掉：生产上会有大量专属用例与场景，"
                ...
                "本版改成 <code>scripts/generated/&lt;scenario_id&gt;/&lt;case_id&gt;.py</code>，"
                "并用 <code>index.json</code> 把 <code>scenario_id → case_id → 脚本 → 数据集</code> 串起来；"
                "generate 用<b>四指纹</b>（用例 / 场景 / 探测结果 / 渲染模板）判定 <b>只重做「变了」的那些</b> —— "
                "无变化时零重写（实测：重写 0 · 跳过 19）。",
        added=[
            "<b>按 id 分文件的产物</b>：<code>scripts/generated/&lt;场景&gt;/&lt;用例&gt;.py</code>"
            "（手搓用例在 <code>generated/manual/</code>）；<code>cases/</code> 同步按场景分目录",
            "<b><code>index.json</code> 索引</b>：case_id → 脚本路径 + 四指纹，是增量判定与产物对账的唯一依据",
            "<b>增量开关</b>：generate 默认 <code>--changed</code>，另有 <code>--force</code>（全量）与 "
            "<code>--only &lt;case_id,...&gt;</code>（只重做指定用例，其余一动不动）",
            "<b>共享运行时拆分</b>：<code>generated/_harness.py</code>（辅助函数）+ "
            "<code>generated/conftest.py</code>（只转发夹具）—— 消除「两个 conftest」的 import 歧义",
            "<b>二类共享助手</b>：<code>tests/featureTest/_gen_layout.py</code>（case_id → 新脚本路径）",
            "<b>新判据</b>：布局契约（唯一脚本 / index↔磁盘双向一致 / 四指纹齐 / 无平铺残留）、"
            "pytest 真收集对账、增量只重写变化项、打包必需项覆盖新目录",
        ],
        changed=[
            "<code>cli run</code> 直接跑 <code>scripts/generated/</code>；<code>explore --verify</code> "
            "用 <code>index.json</code> 取 node id（不再拼 <code>test_cases.py::test_x</code>）",
            "<code>build_tools/pack_release.py</code>：必需项与产物检查改指分模块产物"
            "（三条语义不变：无 pytest.fail 存根 / 走就绪契约 <code>_goto</code> / 无裸 <code>page.goto</code>）",
            "全部判据按新布局读产物（语义未变，另加严数处：逐模块判就绪契约、数据集也不许落盘、"
            "模板↔产物逐字节互锁）；判据数 508 → 537",
            "README 与 <code>docs/training.html</code> 的<b>当前结构</b>描述同步（历史变更记录未动）",
        ],
        fixed=[
            "<b>产物文件名不匹配 pytest 默认收集模式</b> ⇒ 一条都收不到 ⇒ 补 "
            "<code>pytest_collect_file</code> 钩子（新判据当场抓出）",
            "<b>指纹缺「模板」维度</b> ⇒ 改了生成逻辑产物却不刷新（静默不生效）⇒ 补第四指纹",
            "<code>_harness.py</code> 的相对路径按新位置推导（<code>BASE</code> 与 <code>datasets/</code>）",
            "二类若干脚本仍按平铺路径找用例（归档 / 保留策略 / 数据组 / 离线链工具 / 打包计数）⇒ 全部改递归",
            "replay 的「还原」曾写回<b>平铺</b>位置（看着还原了、位置错了）⇒ 改按用例自己的 "
            "<code>scenario_id</code> 写回正确目录",
        ],
    ),
    dict(
        version="8.2.4", date="2026-09-23", tag="上一版本",
    ...
    dict(
        ...
        summary="本版<b>零代码行为变化</b>，只补文档：培训页新开一章，讲清本框架支持的<b>两种业务场景</b>"
                "对应<b>两种手搓文件</b> —— <code>scenarios/&lt;模块&gt;/&lt;id&gt;.yml</code>（自然语言场景）"
                "与 <code>cases/&lt;id&gt;.json</code>（结构化用例）。两条链路在 <code>cases/*.json</code> "
                "<b>汇合</b>，那是它们唯一的接口，也是「AI 产的用例」与「你手搓的用例」能跑同一条执行链的原因。<br>"
                "整章按「<b>怎么手搓出来</b>」组织：字段表 / 动作表 / 断言表 / 两个真实样例 / "
                "五步操作流程 / 六条团队约定 / 一页速查。",
        added=[
            "<b>培训页第 4 章《场景文件与用例文件：怎么手搓（两种业务场景）》</b>："
            "① 两条链路对比（花不花 token / 什么时候用哪个 / 怎么把 AI 用例固化成回归资产）；"
            "② 场景文件字段表（谁读 / 干什么）+ 一份字段注释完整的真实样例 + 四条写作要点 + demo 三件事 + 五步流程；"
            "③ 用例文件五个顶层字段 + <b>7 个动作</b> + <b>9 类断言</b> + 一个真实用例模板 + 五步流程；"
            "④ 两个文件的<b>六条共同约定</b>（只改源头不改产物 / 断言必须操作前可确定 / 不许跨用例依赖 / "
            "定位不唯一要如实报错 / 元素口径对齐生产 / 交付前三条自检）；⑤ 一页速查表",
            "<b>语义名（<code>element</code>）的来源写清楚了</b>：它不是 selector 也不是 testid，"
            "而是页面可被「读」到的身份（<code>placeholder</code> → <code>role+name</code> → 可见文本 → "
            "<code>title</code>/<code>aria-label</code>）；查法 = 先跑 <code>cli probe</code>，"
            "照 <code>output/element_maps/probe_*.json</code> 抄，别猜",
        ],
        changed=[
            "培训页章节编号顺延：原第 4~9 章 → 第 5~10 章（并同步修正章内两处章节交叉引用）",
            "版本号 <code>8.2</code> → <code>8.2.1</code>（单一来源不变）",
        ],
        fixed=[],
        notes=[
            "<b>兼容性</b>：纯文档版，<b>零代码行为变化</b> —— CLI 参数、用例格式、报告形态、判据口径全部不变，"
            "升级只需替换代码包（不必重跑 <code>generate</code>）。",
            "真值：一类 <b>421 passed / 0 red</b> · <code>verify_html_sync</code> 逐字节通过 · "
            "培训页 <b>209.5 KB</b>（新增约 24 KB）。",
        ],
    ),
    dict(
        version="8.2", date="2026-09-22", tag="上一版本",
        ...
        summary="把「埋点只顶层有」从<b>口头约定</b>变成<b>可执行判据</b>，并按这个口径改造 demo、"
                ...
                "可用性、正确性、健壮性就都是空谈。",
        added=[
            "<b>判据规则 ⑤⑥</b>（<code>tests/frameworkTest/testid_policy.py</code>）："
    ...
    dict(
        ...
        notes=[
            "判据：一类 <code>pytest tests/frameworkTest/ -q</code> <b>238 passed</b>（与重构前逐条一致）· "
            "端到端 <code>cli run</code> 19 个节点<b>分批</b>跑 <b>19 passed / exit 0</b> · "
            "二类 <code>bash tests/featureTest/run_verifications.sh</code> 全部 verify_*.py（条数以 <code>--list</code> 输出为准）· "
    ...
    dict(
        ...
        added=[
            "<b>特性验证统一入口</b> <code>tests/featureTest/run_verifications.sh</code>：逐个跑 "
            "<code>verify_*.py</code>、内存不足<b>如实 SKIP</b>（不硬跑、不当通过）、跑完出汇总表；"
            "支持 <code>--only</code> / <code>--list</code> / <code>--no-demo</code>",
            "<b>R8 判据</b> <code>tests/featureTest/verify_html_sync.py</code>：培训页必须①可复现（换哈希种子 md5 相同）"
    ...
    dict(
        ...
        added=[
            ...
            "回归：<code>tests/frameworkTest/test_name_alignment.py</code> 11 条（含「提示词里必须真出现这段披露」的"
        ...
        notes=[
            ...
            "回归口径：<code>pytest tests/frameworkTest/ -q</code> <b>158 passed</b> · <code>cli run</code> "
    ...
    dict(
        ...
        added=[
            ...
            "<code>tests/frameworkTest/test_case_quality_gate.py</code>（13 条，秒级不需 demo）：11 种断言 kind × 7 种期望值形态 × "
        ...
        changed=[
            ...
            "<code>tests/featureTest/verify_cross_page.py</code>：第二节新增「弱证据必被红线拦 + 真证据不许误拦」两条判据；"
        ...
        fixed=[
            ...
            "<b>负向验证段静默失效</b>：<code>tests/featureTest/verify_cross_page.py</code> 的负向段<b>自 V7.5.1 起就没跑起来</b>"
        ...
        notes=[
            "<b>自测与端到端证据（都真跑）</b>：<code>pytest tests/frameworkTest/ -q</code> → <b>143 passed</b>"
    ...
    dict(
        ...
        notes=[
            ...
            "<b>自测证据</b>：<code>pytest tests/frameworkTest/ -q</code> → 130 passed；<code>pack_release.py --check</code> 通过；"
    ...
    dict(
        ...
        added=[
            ...
            "<b>交付前自检</b>：① <code>tests/frameworkTest/test_artifacts_health.py</code>（未映射=0 · 无裸 "
            ...
            "<code>tests/frameworkTest/test_generate_quality_gate.py</code> · <code>tests/frameworkTest/test_target_reachability.py</code> · "
            "<code>tests/frameworkTest/test_pack_release.py</code>：三个「测试的测试」（不启浏览器、秒级）",
        ],
        changed=[
            ...
            "<code>tests/featureTest/verify_slow_target.py</code> · <code>tests/featureTest/verify_picker_layer.py</code> 补 "
        ...
        notes=[
            ...
            "<b>自测证据（本机）</b>：<code>pytest tests/frameworkTest/ -q</code> → <b>130 passed</b>；"
    ...
    dict(
        ...
        added=[
            ...
            "<b>慢目标闸门 <code>tests/featureTest/verify_slow_target.py</code></b>：自起 demo + 慢代理，"
            "用同一批生成脚本指向代理跑关键用例，必须全绿；内存不足时明确 SKIP（exit 3），<b>不报假绿</b>",
            "<b><code>HYBRID_BASE_URL</code> 目标地址覆盖</b>：同一套用例可跑本机 / 慢代理 / 预发（闸门与多环境都靠它）",
            "<code>tests/frameworkTest/test_ready_and_locate.py</code>：13 条契约锁（模板 ⇄ 生成物互锁 + CLI 降级行为实测，秒级不需浏览器）",
        ],
        changed=[
    ...
    dict(
        ...
        fixed=[
            ...
            "<code>tests/frameworkTest/test_utf8_io.py</code> 的 <code>SKIP_DIRS</code> 去掉已不存在的 "
    ...
    dict(
        ...
        added=[
            ...
            "<b>tests/featureTest/verify_picker_layer.py</b>：弹层（picker）端到端回归 —— 弹层里 6 个同名「选择」按钮"
            "必须<b>按所在行命名</b>收集到、探完必须把层关掉、轮询必须第一轮就命中",
            "<b>tests/featureTest/verify_cross_page.py 新增第四段</b>：UI 新建 → 详情页读到的客户必须等于弹层里选的那个；"
            "对不存在的编号必须如实报「未找到」",
    ...
    dict(
        ...
        added=[
            ...
            "<b>tests/frameworkTest/test_utf8_io.py</b>（10 条）：含<b>用 zh_CN.gbk locale 精确复现AprilPark1012那条报错</b>"
    ...
    dict(
        ...
        added=[
            ...
            "<b>tests/frameworkTest/test_cli_flags.py</b>：参数契约回归（约 3 秒，不需要 demo / 不需要 AI key）——"
...
ARTICLE_HANDCRAFT = """<!-- ========== 4. 场景文件与用例文件：怎么手搓（两种业务场景） ========== -->
<section>
  <h2 class="sec-title"><span class="n">4</span>场景文件与用例文件：怎么手搓（两种业务场景）</h2>
  <p class="sec-sub">
    本框架支持<b>两种业务场景</b>，对应<b>两种手搓文件</b>——<b>两个文件都是人写的</b>，这是团队要记住的第一条约定。
    两条链路在 <span class="code-inline">cases/*.json</span> <b>汇合</b>：那是它们<b>唯一的接口</b>。
    汇合之后完全同一条链（<span class="code-inline">generate</span> → <span class="code-inline">run</span>），
    这就是「<b>AI 产的用例</b>」和「<b>你手搓的用例</b>」能跑同一条执行链、拿到同一份报告的原因。
  </p>

  <h3 style="margin:26px 0 6px">4.1 先分清两条链路（决定你要写哪个文件）</h3>
  <table class="tbl">
    <tr><th style="width:11%"> </th><th style="width:44.5%">场景① · 场景驱动（AI 语义识别）</th><th>场景② · 用例驱动（直接写用例）</th></tr>
    <tr>
      <td><b>你手搓的文件</b></td>
      <td><span class="code-inline">scenarios/&lt;模块&gt;/&lt;场景id&gt;.yml</span><br>
          自然语言动作流（人话）</td>
      <td><span class="code-inline">cases/manual/&lt;用例id&gt;.json</span><br>
          结构化步骤 + 断言（机器话）</td>
    </tr>
    <tr>
      <td><b>入口命令</b></td>
      <td><span class="code-inline">python -m framework.cli explore --ai --scenario-file …</span></td>
      <td><span class="code-inline">python -m framework.cli generate &amp;&amp; … run</span></td>
    </tr>
    <tr>
      <td><b>产出</b></td>
      <td><span class="code-inline">cases/&lt;场景id&gt;/ai_&lt;场景id&gt;_&lt;HHMMSS&gt;.json</span>（AI 写的用例，按场景分目录）</td>
      <td>直接进 <span class="code-inline">scripts/generated/&lt;场景id&gt;/&lt;用例id&gt;.py</span>（无中间产物）</td>
    </tr>
    <tr>
      <td><b>花不花 token</b></td>
    ...
    <tr>
      <td><b>什么时候用</b></td>
      <td>页面/口径已清楚，要<b>批量铺覆盖</b>（一个场景 × N 组数据 = N 条用例）；
          或者你想先让 AI 试一遍看它怎么选控件</td>
      <td><b>边界与负向</b>（点错该失败、空值该拦截）、要<b>精确断言</b>、临时补一条、
          以及把 AI 产出的满意用例<b>固化</b>成回归资产</td>
    </tr>
    <tr>
      <td><b>推荐节奏</b></td>
      <td>先写场景跑通 → 看 AI 产出的用例 → 满意就<b>固化</b>成 <span class="code-inline">cases/&lt;id&gt;.json</span>（去掉 <span class="code-inline">ai_</span> 前缀）→ 之后当回归资产零成本重跑</td>
      <td>抄一份<b>最像</b>的现成用例改（别从零写）</td>
    </tr>
  </table>
  <p style="color:var(--muted);font-size:.9rem;margin-top:8px">
    一句话：<b>场景① 前期花 token 换覆盖速度；场景② 零成本换精确度</b>。两者产出的用例格式完全相同，随时可以互相转换。
  </p>

  <h3 style="margin:26px 0 6px">4.2 场景文件 <span class="code-inline">scenarios/&lt;模块&gt;/&lt;场景id&gt;.yml</span></h3>
  <p>
    <b>一个文件 = 一个场景；文件名 = 场景 id</b>（小写蛇形，正则 <span class="code-inline">^[a-z][a-z0-9_]*$</span>）。
    目录按业务模块分（如 <span class="code-inline">scenarios/contracts/</span>、<span class="code-inline">scenarios/orders/</span>）。
    <b>只有 <span class="code-inline">scenario</span> 字段是必填的</b>，其余按需。
  </p>

  <h4 style="margin:18px 0 4px">字段表（谁读 / 干什么）</h4>
  <table class="tbl">
    <tr><th style="width:26%">字段</th><th style="width:12%">谁读</th><th>说明</th></tr>
    <tr><td><span class="code-inline">scenario</span> ★必填</td><td><b>AI</b></td><td>自然语言动作流（YAML 块标量 <span class="code-inline">|</span> 支持多行）。<b>这是喂给 AI 的全部动作意图</b>，写得越具体，挑控件越准</td></tr>
    <tr><td><span class="code-inline">id</span></td><td>框架/人</td><td>稳定标识（缺省取文件名）；用于 <span class="code-inline">case_id</span> 命名与溯源 —— <b>它就是产出用例的 id 前缀</b></td></tr>
    <tr><td><span class="code-inline">title</span> / <span class="code-inline">description</span></td><td>人</td><td>标题与说明；报告里会显示，等于给同事看的注释</td></tr>
    <tr><td><span class="code-inline">owner</span> / <span class="code-inline">updated</span> / <span class="code-inline">notes</span></td><td>人</td><td>责任人 / 更新时间 / 备注（<span class="code-inline">owner</span> 建议写你的 GitHub handle）</td></tr>
    <tr><td><span class="code-inline">tags</span></td><td>框架</td><td>批量筛选用：<span class="code-inline">--tag smoke</span>（可重复，OR 语义）</td></tr>
    <tr><td><span class="code-inline">priority</span></td><td>框架</td><td><span class="code-inline">P0/P1/P2</span>；目录批量时按此排序</td></tr>
    <tr><td><span class="code-inline">target.url</span></td><td>框架</td><td>缺省用全局配置 <span class="code-inline">config.TARGET_URL</span></td></tr>
    <tr><td><span class="code-inline">target.page</span></td><td>AI</td><td>页面的人话描述（喂 prompt，帮 AI 认页面）</td></tr>
    <tr><td><span class="code-inline">target.preconditions</span></td><td>人/AI</td><td>前置条件；<b>一期只作为 AI 上下文，不自动执行</b>（别指望它帮你起服务）</td></tr>
    <tr><td><span class="code-inline">business_context</span></td><td>AI</td><td><b>领域知识：字段含义、编号规则、易踩的坑</b>。把 placeholder、可选值、编码规则写进去 —— 这是提升 AI 准确率最划算的一处投入</td></tr>
    <tr><td><span class="code-inline">assert_guard.must_contain</span></td><td>AI + 质量闸</td><td>断言必须围绕的<b>稳定文本</b>（操作前可确定的）。<b>双向生效</b>：事前告诉 AI 该围绕什么写断言，事后落盘硬校验断言是否真的验到了它</td></tr>
    <tr><td><span class="code-inline">assert_guard.forbidden</span></td><td>AI + 质量闸</td><td>本场景禁止出现在断言里的内容（场景专属禁令）</td></tr>
    <tr><td><span class="code-inline">data</span></td><td>框架</td><td><b>参数化：一组数据 = 一条独立用例</b>（报告里独立一行、失败能定位到具体数据集）。写法见下</td></tr>
  </table>
...
# —— 数据参数化：一组数据 = 一条用例（报告里独立一行）——
# 占位符写在 scenario 文案里，这里每组给一组值；
# 组值必须**恰好**覆盖文案里用到的占位符（少了会被拦）。
# id 可选：给了就进报告里的用例名（如 test_xxx[编号-1007]），便于单组定位。
data:
  - {id: "编号-1005", 关键词: "1005", 期望编号: "HT-1005"}
  - {id: "编号-1007", 关键词: "1007", 期望编号: "HT-1007"}

# —— 领域上下文：帮 AI 挑对控件、判断数据规则 ——
business_context: |
...
assert_guard:
  ...
  <h4 style="margin:18px 0 4px">写场景的四条写作要点（血泪教训）</h4>
  <ol style="line-height:1.85">
    <li><b>断言必须「操作前可确定」</b>：写记录标识（<span class="code-inline">HT-1005</span>）或页面固定文案，
        <b>别写运行时才知道的计数</b>（<span class="code-inline">共 N 条</span>）。全局已禁计数/时间戳/随机值，
        <span class="code-inline">assert_guard</span> 里可再加本场景专属禁令。</li>
    <li><b><span class="code-inline">business_context</span> 越具体，AI 挑控件越准</b>：把 placeholder、可选值、编号规则写进去。</li>
    <li><span class="code-inline">must_contain</span> 会<b>双向生效</b>：事前告诉 AI 该围绕什么写断言，事后校验断言是否真的验到了它。</li>
    <li><b>场景跑不通时，先改场景文件</b>（这是正常的编写循环），<b>而不是去改框架</b>。
        只有当「框架真的缺能力」时才动框架 —— 判断标准：同一个卡点是否在多个场景反复出现。</li>
  </ol>

  <h4 style="margin:18px 0 4px">写场景前必知的 demo 三件事</h4>
  <ol style="line-height:1.85">
    <li><b>数据在服务端</b>：列表页与详情页读的是<b>同一条真实记录</b> ⇒ 预置数据的字段跨页一致，可以断言；
        查不到的编号会如实报「未找到」，<b>不会编数据</b>。</li>
    <li><b>每条用例开始前框架会复位数据</b> ⇒ 列表页固定 20 条预置；
        <b>别写「上一条用例新建的数据留给下一条用」这种跨用例依赖</b>（复位后就不存在了）。</li>
    <li><b>客户字段是「弹层列表里选一行」</b>：弹层里多个「选择」按钮文字完全相同 ⇒ 语义名形如
        <span class="code-inline">选择@北京华信科技有限公司</span>；列表页客户筛选是<b>右模糊（前缀）匹配</b>（非前缀不命中）。</li>
  </ol>

  <h4 style="margin:18px 0 4px">手搓一个场景：五步</h4>
  <pre class="tree"># ① 先看被测页面有哪些控件（语义名的唯一来源）
...
# ④ 看产出：用例 + 报告
ls cases/ai_&lt;你的场景id&gt;_*.json
ls log/&lt;run_id&gt;/                          # 用例日志 / 报告 / trace

# ⑤ 不满意 → 回去改 yml（改 scenario 文案或 business_context），再跑第③步
#    满意   → 固化成手搓用例（见 4.3），之后零 token 重跑
</pre>
  <p style="color:var(--muted);font-size:.9rem">
    批量场景：<span class="code-inline">explore --ai --scenario-dir scenarios/ --tag smoke</span>（可加 <span class="code-inline">--limit N</span> 限流）。
    三个入口 <span class="code-inline">--scenario</span> / <span class="code-inline">--scenario-file</span> / <span class="code-inline">--scenario-dir</span>
    <b>互斥</b>，同时给会报错退出（exit 2）。内联 <span class="code-inline">--scenario "…"</span> 适合临时想法，不建议进场景库。
  </p>

  <h3 style="margin:26px 0 6px">4.3 用例文件 <span class="code-inline">cases/&lt;用例id&gt;.json</span></h3>
  <p>
    顶层<b>只有 5 个字段</b>：<span class="code-inline">case_id</span>（唯一 id，也是产物里测试函数名的来源）、
    <span class="code-inline">name</span>（报告里显示的名字）、<span class="code-inline">base_url</span>、
    <span class="code-inline">steps</span>（动作序列）、<span class="code-inline">asserts</span>（断言序列）。
    AI 产的用例（<span class="code-inline">ai_*.json</span>）和你手搓的<b>格式完全一样</b>。
  </p>

  <h4 style="margin:18px 0 4px">steps：支持的 7 个动作（照抄即用）</h4>
  <table class="tbl">
    <tr><th style="width:16%">op</th><th style="width:30%">必填</th><th>说明</th></tr>
    <tr><td><span class="code-inline">goto</span></td><td><span class="code-inline">url</span></td><td>打开页面（一般每个用例第一步）</td></tr>
    <tr><td><span class="code-inline">click</span></td><td><span class="code-inline">element</span></td><td>点击；<span class="code-inline">element</span> 是<b>语义名</b>（见下）</td></tr>
    <tr><td><span class="code-inline">fill</span></td><td><span class="code-inline">element</span> + <span class="code-inline">value</span></td><td>填输入框；<span class="code-inline">value</span> 支持 <span class="code-inline">{datetime}</span> 一类占位符</td></tr>
    <tr><td><span class="code-inline">select</span></td><td><span class="code-inline">element</span> + <span class="code-inline">value</span></td><td>下拉选择</td></tr>
    <tr><td><span class="code-inline">check</span></td><td><span class="code-inline">element</span></td><td>勾选/取消勾选（幂等，不要求当前状态）</td></tr>
    <tr><td><span class="code-inline">press_enter</span></td><td><span class="code-inline">element</span></td><td>在输入框按回车触发（表单提交/搜索）</td></tr>
    <tr><td><span class="code-inline">expect_text</span></td><td><span class="code-inline">text</span></td><td>文本类断言动作（AI 产出路径会用它落断言）</td></tr>
  </table>
  <p style="color:var(--muted);font-size:.9rem;margin-top:6px">
    每个 step 都建议写 <span class="code-inline">desc</span>（人话描述）—— 它会进用例日志，<b>失败时是给同事看的第一手线索</b>。
  </p>

  <h4 style="margin:18px 0 4px">asserts：断言类型（kind）</h4>
  <table class="tbl">
    <tr><th style="width:16%">kind</th><th style="width:32%">写什么</th><th>语义</th></tr>
    <tr><td><span class="code-inline">count</span></td><td><span class="code-inline">selector</span> + <span class="code-inline">expect</span></td><td>命中行数/元素数等于 N（<b>最常用</b>，如「筛完只剩 1 条」）</td></tr>
    <tr><td><span class="code-inline">text</span></td><td><span class="code-inline">element</span> 或 <span class="code-inline">selector</span> + <span class="code-inline">expect</span></td><td>元素文本包含/等于期望值</td></tr>
    <tr><td><span class="code-inline">value</span></td><td>同上</td><td>输入框的当前值</td></tr>
    <tr><td><span class="code-inline">visible</span> / <span class="code-inline">hidden</span></td><td><span class="code-inline">element</span> 或 <span class="code-inline">selector</span></td><td>元素可见 / 不可见（弹层开合、错误提示是否出现）</td></tr>
    <tr><td><span class="code-inline">enabled</span> / <span class="code-inline">disabled</span></td><td>同上</td><td>按钮可点 / 置灰（<b>负向用例主力</b>：必填没填时提交该置灰）</td></tr>
    <tr><td><span class="code-inline">checked</span> / <span class="code-inline">unchecked</span></td><td>同上</td><td>勾选框状态</td></tr>
    <tr><td><span class="code-inline">attr</span></td><td><span class="code-inline">selector</span> + <span class="code-inline">name</span> + <span class="code-inline">expect</span></td><td>属性值（如 <span class="code-inline">data-field</span>）</td></tr>
    <tr><td><span class="code-inline">url</span></td><td><span class="code-inline">expect</span></td><td>当前地址（跳转/带参跳转）</td></tr>
    <tr><td><span class="code-inline">first_row</span></td><td><span class="code-inline">selector</span> + <span class="code-inline">expect</span></td><td>取第一行做比对（排序类断言）</td></tr>
  </table>
  <p style="color:var(--muted);font-size:.9rem;margin-top:6px">
    断言可以<b>多条并存</b>（一个用例 = 一串断言），失败时报告会指出<b>是哪一条</b>。断言是<b>唯一</b>的判定依据 ——
    操作步骤成功不等于测试通过。
  </p>

  <h4 style="margin:18px 0 4px">⚠️ 最关键的一处约定：<span class="code-inline">element</span>（语义名）从哪来</h4>
  <p>
    <b>语义名不是 selector，也不是 testid</b>，而是「页面上这个控件叫什么」的<b>稳定说法</b>：
  </p>
  <pre class="tree">语义名 = 页面上可被"读"到的身份，按优先级取：
        placeholder  →  role + name  →  控件可见文本  →  title / aria-label
        例：'请输入合同名称'（placeholder）· '搜索'（按钮文本）· '选择@北京华信科技有限公司'（弹层选行）
</pre>
  <p style="margin-top:8px">
    <b>怎么查到它们</b>：跑 <span class="code-inline">python -m framework.cli probe</span>，
    它把页面（含<b>弹层及其内嵌挑选层</b>）的可交互元素翻译成语义清单，落到
    <span class="code-inline">output/element_maps/probe_*.json</span>。
    <b>手搓用例前先跑一次 probe，照着清单抄语义名</b> —— 别猜。
  </p>
  <p style="margin-top:8px;padding:10px 12px;border-left:3px solid #d97706;background:rgba(217,119,6,.07);border-radius:6px">
    <b>口径与生产一致</b>：被测系统里<b>只有顶层容器带 testid，控件层没有</b>。
    所以手搓时<b>不要指望用 testid 定位控件</b>；语义名命中不唯一时，框架会<b>如实报歧义并要求你给更精确的定位</b>
    （加容器锚点/字段步/<span class="code-inline">title</span>），<b>不会按顺序猜第一个</b>——这是刻意设计，避免"点错元素还绿"。
  </p>

  <h4 style="margin:18px 0 4px">手搓一个用例：五步</h4>
...
#    ⚠️ 映射不到语义名 → 会打印 "⚠️ 仍未映射" 并让那一步 pytest.fail（绝不静默跳过）

# ④ 只跑你这一条（按 case_id 过滤）
python -m pytest scripts/generated/ -q -k &lt;你的用例id&gt;

# ⑤ 看报告与证据
ls log/&lt;run_id&gt;/          # 用例日志 / report.html / trace
</pre>
  ...
  "steps": [
    ...
    { "op": "fill",      "desc": "在搜索框输入'合同7'",
      "element": "合同编号_名称_管理单元_合同类型_帐套", "value": "合同7" },
    { "op": "press_enter", "desc": "在搜索框按回车触发查询",
      "element": "合同编号_名称_管理单元_合同类型_帐套" },
    { "op": "fill",      "desc": "客户筛选框输入前缀『北京』（右模糊：两家命中）",
      "element": "客户名称_右模糊_前缀匹配", "value": "北京" },
    { "op": "click",     "desc": "点击搜索按钮", "element": "搜索" }
  ],
  "asserts": [
    { "kind": "count", "desc": "关键字『合同7』× 客户前缀『北京』 ⇒ 1 条",
      "selector": "#tbody-contracts tr:has(td[data-field='contractNo'])", "expect": 1 }
  ]
}
</pre>

  <h3 style="margin:26px 0 6px">4.4 两个文件的共同约定（团队要背的六条）</h3>
  <table class="tbl">
    <tr><th style="width:30%">约定</th><th>为什么 / 怎么做</th></tr>
    <tr>
      <td><b>① 只改源头，不改产物</b></td>
      <td><span class="code-inline">scripts/generated/</span>（一个用例一个脚本：
          <span class="code-inline">&lt;场景id&gt;/&lt;用例id&gt;.py</span> + <span class="code-inline">manual/</span> +
          <span class="code-inline">_harness.py</span> + <span class="code-inline">conftest.py</span> + <span class="code-inline">index.json</span>）、
          <span class="code-inline">scripts/datasets/*.json</span> 都是 <span class="code-inline">generate</span> 的<b>产物</b>，
          手改会被下一次 generate 覆盖。要改行为 → 改 <span class="code-inline">cases/*.json</span> 或 <span class="code-inline">scenarios/*.yml</span>，然后重新 generate。</td>
    </tr>
    <tr>
      <td><b>② 断言必须「操作前可确定」</b></td>
      <td>禁计数（<span class="code-inline">共 N 条</span>）、时间戳、随机值 —— 这类断言要么永远为真（假绿），要么随机红。<br>
          写记录标识（<span class="code-inline">HT-1005</span>）、固定文案、或<b>你自己造的数据</b>。</td>
    </tr>
    <tr>
      <td><b>③ 不许跨用例依赖</b></td>
      <td>每条用例开始前框架<b>复位数据</b> ⇒「上一条建的记录」不存在。需要前置数据就<b>在自己的 steps 里造</b>。</td>
    </tr>
    <tr>
      <td><b>④ 定位不唯一要如实报错</b></td>
      <td>语义名命中多个 ⇒ 框架报歧义、要求更精确的定位（容器锚点 / 字段步 / <span class="code-inline">title</span>），
          <b>绝不按顺序猜</b>。你看到歧义报错时，是它<b>在保护你</b>，不是它坏了。</td>
    </tr>
    <tr>
      <td><b>⑤ 元素口径对齐生产</b></td>
      <td>只有顶层容器有 <span class="code-inline">testid</span>，控件层没有 ⇒ 手搓时用语义名，不用 testid 定位控件。</td>
    </tr>
    <tr>
      <td><b>⑥ 交付前自检</b></td>
      <td><span class="code-inline">python -m framework.cli generate</span>（必须 0 未映射）→
          <span class="code-inline">python -m pytest tests/frameworkTest/</span>（一类）→
          <span class="code-inline">python tests/featureTest/run_verifications.py</span>（二类）。
          三条全绿才算改完。</td>
    </tr>
  </table>

  <h3 style="margin:26px 0 6px">4.5 一页速查</h3>
  <table class="tbl">
    <tr><th style="width:34%">我想干什么</th><th style="width:30%">写哪个文件</th><th>命令</th></tr>
    <tr><td>用自然语言批量铺覆盖</td><td><span class="code-inline">scenarios/&lt;模块&gt;/&lt;id&gt;.yml</span></td>
        <td><span class="code-inline">cli explore --ai --scenario-file …</span></td></tr>
    <tr><td>把 AI 产出的用例固化成回归资产</td><td><span class="code-inline">cases/&lt;id&gt;.json</span>（去掉 <span class="code-inline">ai_</span> 前缀）</td>
        <td>改名 + <span class="code-inline">cli generate</span></td></tr>
    <tr><td>补一条边界/负向用例</td><td><span class="code-inline">cases/&lt;id&gt;.json</span></td>
        <td>抄现成的改 → <span class="code-inline">cli generate</span> → 单跑 <span class="code-inline">-k</span></td></tr>
    <tr><td>一组数据跑多条用例</td><td>场景文件的 <span class="code-inline">data:</span> 段</td>
        <td><span class="code-inline">cli explore --ai …</span>（每组一条用例）</td></tr>
    <tr><td>查某个控件叫什么</td><td>—</td><td><span class="code-inline">cli probe</span> → <span class="code-inline">output/element_maps/probe_*.json</span></td></tr>
    <tr><td>只跑我刚写的那条</td><td>—</td><td><span class="code-inline">pytest scripts/generated -q -k &lt;case_id&gt;</span></td></tr>
    <tr><td>跑不通，想确认是环境还是用例</td><td>—</td><td>看 <span class="code-inline">log/&lt;run_id&gt;/</span> 里该用例日志 + <span class="code-inline">report.html</span> + trace</td></tr>
  </table>
</section>
...
def _svg_two_scenarios() -> str:
    ...
    node(x2[0], Y3, W2, 120, C2, BG2, "1", "输入：手写用例",
         ["cases/manual/<case_id>.json", "element 写 semantic_name", "value 可写 {datetime}",
          "（格式与 AI 用例一致）"])
    ...
    node(x2[2], Y4, W2, 120, C2, BG2, "6", "生成产物",
         ["scripts/generated/（脚本）", "datasets/*.json（数据）",
          "_harness.py + conftest.py", "零 token，可进 CI；断言 11 种 kind"])
...
├── <b>cases/</b>                  ★ 自然语言用例(写死数据;按 id 分文件,一个用例一个 json)
│   ├── manual/                      手写用例(search_*.json 模糊/混合搜索 · create_bu_*.json 新建
│   │                                · assert_kinds_*.json 11 种断言 · cross_page_detail.json 跨页)
│   └── &lt;场景id&gt;/                    🤖 explore --ai 产出的 AI 用例(ai_*.json,按场景分目录,可批量清理)
...
│   ├── generated/                   ★P20 脚本根:一个用例一个文件(便于按 id 管理与增量重生成)
│   │   ├── &lt;场景id&gt;/&lt;用例id&gt;.py       AI 用例脚本(对应 cases/&lt;场景id&gt;/&lt;用例id&gt;.json)
│   │   ├── manual/&lt;用例id&gt;.py         手搓用例脚本(对应 cases/manual/&lt;用例id&gt;.json)
│   │   ├── _harness.py              会话级浏览器池/数据注入/变量池/断言辅助/日志(共享辅助)
│   │   ├── conftest.py              只转发夹具(把 _harness 暴露给 pytest)
│   │   └── index.json               case_id → {{script_path, 三种指纹, template_fingerprint}}
  ...
  <div style="margin-top:12px;">
    <div class="pt"><div class="ico">🧭</div><h4>新员工怎么用这份地图找东西</h4>
      <p>① 想知道某步跑什么逻辑 → 看 <span class="code-inline">framework/</span> 对应模块；<br>
         ② 想改测试数据 → 改 <span class="code-inline">cases/</span> 下的用例 json（再跑 generate 重建 scripts/）；<br>
         ③ 想看脚本长啥样 / 数据在哪 → <span class="code-inline">scripts/generated/</span> + <span class="code-inline">scripts/datasets/</span>；<br>
...
<section>
  ...
  <div class="flow">
    ...
    <div class="flow-step">
      ...
      <p style="margin-top:6px;font-size:.8rem;color:#94a3b8">→ scripts/generated/ + datasets/</p>
    </div>
    <div class="flow-step">
      <span class="bdge" style="background:#0f8f6a">✅ Playwright</span>
...
│
└── <b>cmd_run</b>        → pytest scripts/generated -n N     [✅ Playwright]
...
╚══════════════════════════════════════════════════════════════════════════════════════════════╝
 ...
 <b>③ 翻译 + 数据抽离（脚本 ⇄ 数据分离）</b>
      ...
      ├─ generator.<b>_render_harness()</b> → scripts/generated/_harness.py（会话级浏览器池/数据注入/变量池/断言辅助/日志/自愈）
      ├─ generator.<b>_render_generated_conftest()</b> → scripts/generated/conftest.py（只把夹具转发给 pytest）
      └─ 写索引 → scripts/generated/index.json（case_id → script_path + 三种指纹，供增量重生成）
            产物: <b>scripts/generated/（脚本） + scripts/datasets/*.json（数据）</b>
 ...
 <b>④ 执行（零 token、可进 CI）</b>
      └─ cli.py::main() → <b>cli.cmd_run(workers, headed, force)</b>
           ...
           └─ subprocess: pytest scripts/generated -n N --html=log/&lt;run_id&gt;/report.html
...
【阶段3】生成 locator(分层解析)                    → locator_bridge.resolve_locator()
    ...
    └─ 产物: 一条唯一、稳定的 Playwright locator
             ▼
【阶段4】生成 Playwright 脚本                      → scripts/generated/ + datasets/
...
<section>
  ...
  <div class="card" style="border-left:4px solid var(--brand);margin-bottom:18px">
    ...
    <pre class="tree">一类「框架自验证」  tests/frameworkTest/test_*.py      —— 秒级、<b>不依赖 demo 与浏览器</b>、可离线跑
二类「特性自验证」  tests/featureTest/verify_*.py      —— 端到端、<b>需 demo</b>、<b>必含负向证伪</b>
E2E 三场景        场景1 自然语言→AI 链路（离线回放可证伪）· 场景2 手写用例驱动 · 场景3 录制回放验证
统一入口          python tests/featureTest/run_acceptance.py     ← R7 四项一条命令（便宜先跑 + fail fast；SKIP ≠ 通过）
                  python tests/featureTest/run_verifications.py  ← 二类全量（唯一实现；内存不足如实 SKIP 并 exit 3）</pre>
    <p style="color:var(--muted);font-size:.88rem;margin-top:8px">下面每个特性都会列出<b>它自己的</b>判据文件与条数；条数以实跑输出为准（<code>pytest tests/frameworkTest/ -q</code>、<code>run_acceptance.py</code>）。</p>
  </div>
...
手搓 cases/*.json ──────────────────────────┘        ▲ 缺项才现场 probe；ElementMap 快照优先</pre>
    ...
    <div style="border:1px dashed var(--line);border-radius:8px;padding:10px;font-size:.9rem;margin-top:10px">
      ...
      <ul style="margin:6px 0 0 18px;line-height:1.75">
        <li><b>一类：</b><span class="code-inline">tests/frameworkTest/test_offline_chain_python.py</span>（9 条：离线链 Python 化，跨平台）· <span class="code-inline">tests/frameworkTest/test_no_gate_coupling.py</span>（2 条：框架自测与 demo 解耦）</li>
        <li><b>二类：</b><span class="code-inline">tests/featureTest/verify_offline_delivery.py</span>（交付两件套形态）· <span class="code-inline">tests/featureTest/verify_offline_chain_deps.py</span>（离线链依赖齐备）· <span class="code-inline">tests/featureTest/verify_e2e_scenario3_replay.py</span>（<b>7 项，含 2 条负向证伪 + 归档零残留</b>）</li>
  ...
  <div class="card" style="border-left:4px solid var(--brand);margin-bottom:18px">
    ...
    <div style="border:1px dashed var(--line);border-radius:8px;padding:10px;font-size:.9rem;margin-top:10px">
      <b>🧪 测试验证用例设计</b>
      <ul style="margin:6px 0 0 18px;line-height:1.75">
        <li><b>一类：</b><span class="code-inline">tests/frameworkTest/test_ready_and_locate.py</span>（13 条：ready 契约、_goto/_wait_ready 接线、误导性报错消失）· <span class="code-inline">tests/frameworkTest/test_element_ambiguity_gate.py</span>（15 条）· <span class="code-inline">tests/frameworkTest/test_name_alignment.py</span>（11 条：精确名优先 / 模糊命中留痕 / 歧义必须 fail loud / 严格模式拒绝近似）· <span class="code-inline">tests/frameworkTest/test_import_targets.py</span>（4 条：内部 import 都能解析）</li>
        <li><b>二类：</b><span class="code-inline">tests/featureTest/verify_picker_layer.py</span>（弹层 picker 链回归）· <span class="code-inline">tests/featureTest/verify_order_pages.py</span> / <span class="code-inline">tests/featureTest/verify_order_pick_create.py</span>（真开浏览器点、真读页面与服务端状态，<b>不看「代码里写了」</b>）</li>
  ...
  <div class="card" style="border-left:4px solid var(--brand);margin-bottom:18px">
   ...
   —— 任一层不确定都<b>走失败</b>，宁可报错也不点错元素</pre>
    ...
    <div style="border:1px dashed var(--line);border-radius:8px;padding:10px;font-size:.9rem;margin-top:10px">
      <b>🧪 测试验证用例设计</b>
      <ul style="margin:6px 0 0 18px;line-height:1.75">
        <li><b>一类：</b><span class="code-inline">tests/frameworkTest/test_element_ambiguity_gate.py</span>（15 条）· <span class="code-inline">tests/frameworkTest/test_name_alignment.py</span>（11 条：<b>歧义裸名不许猜</b>、精确名优先、影子名要列出来）· <span class="code-inline">tests/frameworkTest/test_llm_retry.py</span>（3 条：LLM 失败重试边界）</li>
        <li><b>二类：</b><span class="code-inline">tests/featureTest/verify_element_ambiguity.py</span>（<b>12 项，含负向</b>）· <span class="code-inline">tests/featureTest/verify_cross_page.py</span>（跨页场景：<b>每条错误都必须失败</b>——防假绿铁律）</li>
  ...
  <div class="card" style="border-left:4px solid var(--brand);margin-bottom:18px">
    ...
    <pre class="tree">cases/…/xxx.json ──generate──▶  scripts/generated/…/xxx.py（逻辑：操作类型 → Playwright 调用）
  ...
  --debug: 有 DISPLAY ─▶ 真窗口(headed)  |  无 DISPLAY ─▶ 录屏 + 逐步截图 + trace（都不报错）</pre>
    ...
    <div style="border:1px dashed var(--line);border-radius:8px;padding:10px;font-size:.9rem;margin-top:10px">
      <b>🧪 测试验证用例设计</b>
      <ul style="margin:6px 0 0 18px;line-height:1.75">
        <li><b>一类：</b><span class="code-inline">tests/frameworkTest/test_env_adaptation.py</span>（22 条：Windows / 非 git 目录 / 无 demo 下的降级路径）· <span class="code-inline">tests/frameworkTest/test_utf8_io.py</span>（20 条：UTF-8 机理 + 真实默认编码路径）· <span class="code-inline">tests/frameworkTest/test_retention_runs.py</span>（14 条：run-id 日志隔离与保留策略）· <span class="code-inline">tests/frameworkTest/test_cli_exit_codes.py</span>（10 条：退出码契约，跳过 ≠ 通过）</li>
        <li><b>二类：</b><span class="code-inline">tests/featureTest/verify_retention_runs.py</span>（全真跑：日志保留）· <span class="code-inline">tests/featureTest/verify_slow_target.py</span>（慢目标闸门：慢代理 300ms 下 6 条关键用例全绿；<b>内存不足如实 SKIP exit 3</b>）</li>
...
无外网机器:  output/llm_cassettes/ ◀── 解包        ▼
    ...
    <div style="border:1px dashed var(--line);border-radius:8px;padding:10px;font-size:.9rem;margin-top:10px">
      <b>🧪 测试验证用例设计</b>
      <ul style="margin:6px 0 0 18px;line-height:1.75">
        <li><b>一类：</b><span class="code-inline">tests/frameworkTest/test_llm_cassette.py</span>（30 条：键计算 / 归一化 / 多键兼容 / <b>负向：键不匹配必须报「没有这一份」、坏录像文件不许毁掉整次回放</b>）· <span class="code-inline">tests/frameworkTest/test_offline_chain_python.py</span>（9 条）· <span class="code-inline">tests/frameworkTest/test_pack_release.py</span>（含出厂闸门 3 条：接线锁 / 覆盖不全必须拦 / 逃生口必须有效）</li>
        <li><b>二类：</b><span class="code-inline">tests/featureTest/verify_e2e_scenario3_replay.py</span>（场景1 离线端到端，<b>7 项含 2 条负向 + 归档零残留</b>）· <span class="code-inline">tests/featureTest/verify_e2e_scenario3_cassette.py</span>（<b>场景3 = 录制回放验证</b>：覆盖体检 / 体检有效性负向 / 不匹配必须 fail loud / <span class="code-inline">--with-record</span> 时跑「录制→回放闭环」）</li>
  ...
  <div class="card" style="border-left:4px solid var(--brand)">
    ...
    <pre class="tree">python tests/featureTest/run_acceptance.py        ← 一条命令跑完 R7 四项（便宜先跑 + fail fast）
  [0] 闸门自检（先验「闸门自己」）
  [1] ③ demo 新鲜度      [2] ① 框架自测 337 条     [3] ④ E2E 场景3→2→1     [4] ② 特性自测 15 脚本
  退出码: 0 通过 / 3 跳过(不算通过) / 其它失败 —— 任一项红就停手并报「哪一项」</pre>
    <p><b>怎么用：</b><code>python tests/featureTest/run_acceptance.py</code>（日常）· <code>--with-record</code>（发版前含录制闭环）· <code>python tests/featureTest/run_verifications.py</code>（只跑二类）。</p>
    <div style="border:1px dashed var(--line);border-radius:8px;padding:10px;font-size:.9rem;margin-top:10px">
      <b>🧪 测试验证用例设计（闸门自己也要被判据钉住）</b>
      <ul style="margin:6px 0 0 18px;line-height:1.75">
        <li><b>一类：</b><span class="code-inline">tests/frameworkTest/test_acceptance_entry.py</span>（8 条：入口步骤顺序即契约 / 三场景齐全 / 跳过必须标出 / SKIP ≠ 通过）· <span class="code-inline">tests/frameworkTest/test_demo_freshness_gate.py</span>（21 条）· <span class="code-inline">tests/frameworkTest/test_pack_release.py</span>（16 条）· <span class="code-inline">tests/frameworkTest/test_artifacts_health.py</span>（7 条：产物健康，坏产物给人话诊断）· <span class="code-inline">tests/frameworkTest/test_target_reachability.py</span>（7 条：连不上时统一给「先起 demo」指引）· <span class="code-inline">tests/frameworkTest/test_case_quality_gate.py</span>（13 条）</li>
        <li><b>二类：</b><span class="code-inline">tests/featureTest/verify_demo_freshness.py</span>（<b>7 项含负向证伪 + 真重启 + 还原</b>：闸门自己也必须被证明「坏 demo 能抓、好 demo 不误伤」）· <span class="code-inline">tests/featureTest/verify_slow_target.py</span> · <span class="code-inline">tests/featureTest/verify_html_sync.py</span>（R8：培训页可复现 + 与代码逐字节一致）</li>
  ...
  <div class="card" style="border-top:4px solid var(--brand);margin-top:18px">
    ...
    <p style="font-size:.94rem;line-height:1.8">
      ...
      3️⃣ 跑一遍第 6 章图例⓪ 的六个阶段对照代码 → 4️⃣ 遇到点不中元素时回读 <b>特性 2 / 3</b> →
  ...
  <div class="card">
    ...
    <div class="pit"><h4>坑6 · 编码：别依赖「系统默认编码」（中文 Windows 上必崩）</h4>
      <p>实测事故：<code>explore --ai</code>（默认带 <code>--verify</code>）在中文 Windows 上报
         ...
         （<code>cli.main()</code> 已经默认调了）。<code>tests/frameworkTest/test_utf8_io.py</code> 会做 AST 全仓扫描，
...
<span class="prompt">$</span> <span class="cmt">③</span> python -m pytest tests/frameworkTest/ -q            <span class="cmt"># 一类判据</span>
<span class="prompt">$</span>      python tests/featureTest/run_verifications.py   <span class="cmt"># 二类特性验证</span>
</pre>
  <p style="margin:4px 0 10px;color:var(--muted);font-size:.88rem">
...
<span class="prompt">$</span> python -m framework.cli generate       <span class="cmt"># ② 读 cases/ 用例 → 生成 scripts/generated/(脚本) + datasets/(数据抽离)</span>
...
<span class="prompt">$</span> python -m framework.cli explore --ai --scenario "输入1005点搜索确认出现HT-1005"
                                              <span class="cmt"># ④ AI 语义识别 → cases/&lt;场景id&gt;/ai_*.json（默认 --verify 立刻试跑）</span>
<span class="prompt">$</span> python -m framework.cli prune --keep 20  <span class="cmt"># ⑤ 归档保留：快照各留最近 N 个</span>
<span class="prompt">$</span> python -m pytest tests/frameworkTest/ -q               <span class="cmt"># ⑥ 框架自测（一类）：秒级，不需要 demo / key</span></pre>
    <p style="margin-top:10px;color:var(--muted);font-size:.92rem">
    ...
    <p style="margin-top:12px;color:var(--muted);font-size:.92rem;border-left:3px solid #999;padding-left:10px">
      <b>框架自测（一类）的环境前提</b>（2026-09-22 实测口径）：上面那条 <code>pytest tests/frameworkTest/ -q</code>
    ...
    <p style="margin-top:10px;color:var(--muted);font-size:.92rem">
      ...
      <code>python tests/featureTest/run_verifications.py</code>（全部）·
    ...
    <p style="margin-top:10px;color:var(--muted);font-size:.92rem">
      <b>★ R7 四项验收：一条命令跑完</b> —— <code>python tests/featureTest/run_acceptance.py</code>。<br>
