...
SELF = "frameworkTest/test_no_gate_coupling.py"


def _tracked_files() -> tuple[list[str], str]:
    """(文件清单, 清单来源) —— 走 `frameworkTest/repo_files.py`：git 优先、非 git **等效降级**。
...
def test_repo_has_zero_gate_references():
    ...
    assert files, (f"文件清单为空 —— 判据前提不成立（清单来源：{source}）。"
                   f" 若这里是交付包解压目录：非 git 环境本应走降级扫描，请检查 frameworkTest/repo_files.py")
