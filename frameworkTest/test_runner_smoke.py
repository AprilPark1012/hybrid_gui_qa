...
**为什么要有它**（真实事故）：迁移 `tests/` → `frameworkTest/` + `featureTest/` 时，
`run_verifications.py` 里给子导入用的 `sys.path` 只插了 `featureTest/`，
而共享辅助（`demo_freshness.py` 等）住在 `frameworkTest/` ⇒
...
判据：
  ① `python featureTest/run_verifications.py --list` 必须 **exit 0**（能起来、能列出脚本）
  ② 列出的脚本数 == `featureTest/verify_*.py` 的实际数量（收录口径没漏）
...
def test_runner_runs_a_real_script(capsys=None):
    ...
    assert p.returncode == 0, (
        ...
        "多半是 sys.path 没把共享辅助目录（frameworkTest/）带上：\n"
        + "\n".join(out.strip().splitlines()[-12:]))


def test_runner_lists_every_verify_script():
    """收录口径：`--list` 必须列出 featureTest/ 下的**每一个** verify_*.py（不许手写清单漏项）。"""
...
def test_runner_declares_both_helper_paths():
    """契约：runner 的 sys.path 必须同时含 featureTest/ 与 frameworkTest/（辅助住后者）。"""
    ...
    assert re.search(r"sys\.path\.insert\([^)]*frameworkTest", text), (
        ...
        "frameworkTest/，不插进 sys.path 就会 ModuleNotFoundError（2026-09-23 实测踩到）。"
