...
`demo/` 的订单两页 + todo 页、`featureTest/` 的两个入口（`run_verifications.py` / `run_acceptance.py`）
...
跑法（秒级，不需要 demo/key/git）：
    python -m pytest frameworkTest/test_docs_sync.py -v
"""
...
MUST_BE_DOCUMENTED = (
    ...
    # 判据与验证入口（frameworkTest/ + featureTest/）
...
# ② 路径引用：只认「带目录前缀 + 带扩展名」
_PATH = re.compile(
    r"(?<![\w/.-])((?:cases|frameworkTest|featureTest|docs|build_tools|framework|scenarios|demo|scripts|releases)"
...
def test_negative_missing_path_is_detected():
    assert missing_paths("见 <code>frameworkTest/definitely_missing_xyz.py</code>") == \
        ["frameworkTest/definitely_missing_xyz.py"]
