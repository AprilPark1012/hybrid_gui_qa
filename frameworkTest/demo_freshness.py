...
用法（验证入口 / 手工排查都用它）：
    python frameworkTest/demo_freshness.py --check               # 只查：0=fresh / 3=stale / 4=no_process / 5=unknown
    python frameworkTest/demo_freshness.py --ensure              # 查 + 不新鲜就重启 + 等到就绪 + 复检
    python frameworkTest/demo_freshness.py --start-if-missing     # 没起才起
    python frameworkTest/demo_freshness.py --json                 # 机器可读（给脚本调用）
    python frameworkTest/demo_freshness.py --fingerprint          # 只打印当前 demo 源码的内容指纹（排查用）
"""
