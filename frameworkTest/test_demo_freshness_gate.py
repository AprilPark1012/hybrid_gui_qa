...
被测对象：`frameworkTest/demo_freshness.py`。端到端用法见 `featureTest/run_verifications.sh`（跑二类前先 `--ensure`）。
"""
...
def _run(args, cwd):
    return subprocess.run([sys.executable, "frameworkTest/demo_freshness.py", *args], cwd=str(cwd),
...
def test_run_verifications_checks_gate_every_round():
    ...
    只在开头查挡不住这种中途污染。
    """
    src = (Path(__file__).resolve().parents[1] / "featureTest" / "run_verifications.py").read_text(encoding="utf-8")
