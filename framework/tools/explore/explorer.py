...
def _build_planner_prompt(scenario: str, items: list[dict], url: str,
    ...
    prompt += (
        ...
        f"3d. **埋点现实（P16 · 2026-09-22 现场口径）**：真实系统里 **只有顶层容器**（表格/弹层/工具栏/区块）"
        f"通常有 data-testid，行、单元格、行内链接按钮**往往没有** ⇒ 控件清单里每个控件自带的 "
        f"`anchor`（所属容器：kind/by/value）与 `path`（容器内相对路径：row=行锚文本、col=data-field 或表头文本、"
        f"target=目标语义）就是给你描述「从哪个锚点、沿什么路径找到它」的依据。"
        f"**控件没有 test_id 是常态、不是缺陷**：不要因此放弃它，也**不要自己编 CSS/selector**"
        f"（本项目铁律：定位由框架从语义合成）；`path` 为 null 表示该控件不在表格行内（用 anchor + role/name 即可）；\n"
