...
# ⇒ 以后再挪位置只改这一行；漏改会被 tests/frameworkTest/test_cli_flags.py 的判据当场抓住（--version 不许变 unknown）。
...
# ⇒ 凭空多出空目录。已整体删除；防复发见 tests/frameworkTest/test_cli_exit_codes.py::test_ensure_dirs_only_declares_used_dirs。
...
def ensure_dirs():
    ...
    #   防复发：tests/frameworkTest/test_cli_exit_codes.py::test_ensure_dirs_only_declares_used_dirs
