...
执行顺序（**拍板口径：便宜先跑 + fail fast；编号 ≠ 执行顺序**）：
  ...
  [0] gate_selfcheck      闸门自检 `featureTest/verify_demo_freshness.py`（~1s；**红则停手**）
                          —— 先验「闸门自己」：它若坏了，③ 的「新鲜」结论毫无意义，后面全建在沙子上
  [1] demo_freshness      ③ `frameworkTest/demo_freshness.py --ensure --start-if-missing`
  [2] framework_selftest  ① `pytest frameworkTest/ -q`（~15s，**不依赖 demo** —— 一类自测刻意与 demo 解耦）
  [3] e2e                 ④ 三个场景（见下）
  [4] feature_selftest    ② `featureTest/run_verifications.py`（最慢：单个脚本可达 7 分钟 ⇒ 放最后）
...
用法：
    python featureTest/run_acceptance.py                 # 验收全量（场景1 走离线回放）
    python featureTest/run_acceptance.py --list          # 只看会跑哪些（不执行）
    python featureTest/run_acceptance.py --skip-ai       # 跳过场景1 那一步（会如实标「跳过」且不算通过）
    python featureTest/run_acceptance.py --with-record   # 额外跑场景3 的「录制→回放闭环」（要 key + 外网）
    python featureTest/run_acceptance.py --only e2e      # 只跑某一项（排查用）
    python featureTest/run_acceptance.py --keep-going    # 默认遇失败即停（fail fast）；这个开关改成跑完再汇总
    python featureTest/run_acceptance.py --min-mem 400   # 内存阈值（透传给二类入口）
...
HELP_TEXT = {
    ...
    "framework_selftest": "① 框架自测：pytest frameworkTest/ -q（不依赖 demo）",
...
def step_gate_selfcheck(py: str) -> tuple[str, int, str]:
    ...
    rc, tail = _run(py, ["featureTest/verify_demo_freshness.py"], timeout=300)
    return _verdict(rc), rc, tail


def step_demo_freshness(py: str) -> tuple[str, int, str]:
    """[1] ③ demo 新鲜度闸门（不新鲜就自动重启并等到就绪）。"""
    rc, tail = _run(py, ["frameworkTest/demo_freshness.py", "--ensure", "--start-if-missing"], timeout=120)
    return _verdict(rc), rc, tail


def step_framework_selftest(py: str) -> tuple[str, int, str]:
    """[2] ① 框架自测（一类；与 demo 无关）。"""
    rc, tail = _run(py, ["-m", "pytest", "frameworkTest/", "-q"], timeout=900)
    return _verdict(rc), rc, tail
...
def step_e2e(py: str, *, skip_ai: bool, with_record: bool = False) -> tuple[str, int, str, list[tuple[str, int]]]:
    ...
    args3 = ["featureTest/verify_e2e_scenario3_cassette.py"] + (["--with-record"] if with_record else [])
...
def step_feature_selftest(py: str, min_mem: int) -> tuple[str, int, str]:
    """[4] ② 特性自测（二类；最慢）。"""
    rc, tail = _run(py, ["featureTest/run_verifications.py", "--min-mem", str(min_mem)], timeout=None)
    return _verdict(rc), rc, tail
