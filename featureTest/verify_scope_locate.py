...
本脚本就是它的判据：用**零 testid 的夹具页**（`featureTest/fixtures/zero_testid_page.html`）当真环境。
...
sys.path.insert(0, str(REPO / "frameworkTest"))
from framework.tools.common.text_io import force_stdio          # noqa: E402

force_stdio()

FIXTURE = REPO / "featureTest" / "fixtures" / "zero_testid_page.html"
...
def main() -> int:
    ...
    # （frameworkTest/test_import_targets.py）报「import 目标不存在」——那会掩盖真正的搬家漏改，属于污染判据。
    import importlib
    try:
