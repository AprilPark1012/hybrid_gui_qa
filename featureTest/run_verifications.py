...
用法（Windows PowerShell / Linux / macOS 完全一致）：
    python featureTest/run_verifications.py                  # 全量（需要 demo：脚本会自动起/停）
    python featureTest/run_verifications.py --only ambiguity # 只跑名字含该子串的
    python featureTest/run_verifications.py --no-demo        # 不起 demo（只跑自包含的验证）
    python featureTest/run_verifications.py --list           # 只列出会跑哪些
    python featureTest/run_verifications.py --stream         # 实时打印每个子进程的输出（默认只打尾部摘要）
    python featureTest/run_verifications.py --min-mem 400    # 覆盖内存阈值（默认 550MB，也可用 VERIFY_MIN_MEM_MB）

退出码：**0** = 全通过（跳过项会如实列出）· **1** = 有失败 · **2** = 环境 / 用法问题 · **3** = 内存不足整体 SKIP。
⭐ 口径：**SKIP 不是绿灯** —— 内存不够时如实报 3，绝不假装通过。

为什么要有这个 Python 版（2026-09-22 使用者现场反馈）：
`bash featureTest/run_verifications.sh` 在 Windows PowerShell 里**根本跑不起来**
（`无法将"bash"项识别为 cmdlet…` —— 机器上没装 Git Bash / WSL）
⇒ 二类验证对团队里的 Windows 用户等于**不存在**，而本框架的现场恰恰是 Windows。
⇒ 口径：**唯一实现 = 本文件**；`featureTest/run_verifications.sh` 退化成薄包装（转发到这里），
...
- demo 启停：`subprocess.Popen` + 平台各自的后台标志（Windows `DETACHED_PROCESS`，POSIX `start_new_session`）；
  探活用 urllib（与 `frameworkTest/demo_freshness.py` 同一口径）；
...
def list_scripts(repo: Path = REPO, only: str = "") -> list[Path]:
    """自动收录 `featureTest/verify_*.py`（**不许手写清单**，所以不会漏新脚本）。"""
    items = sorted(repo.glob("featureTest/verify_*.py"))
    if only:
...
def _parse(argv: list[str]) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        ...
        description="特性验证统一入口（二类）：逐个跑 featureTest/verify_*.py 并汇总。SKIP 不是通过。",
    )
...
def main(argv: list[str]) -> int:
    ...
    if not args.no_demo:
        ...
        rc = subprocess.run([python, "frameworkTest/demo_freshness.py", "--ensure", "--start-if-missing"],
