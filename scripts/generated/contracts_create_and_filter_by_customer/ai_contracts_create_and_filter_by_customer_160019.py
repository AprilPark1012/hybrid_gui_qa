"""hybrid_gui_qa 用例模块：ai_contracts_create_and_filter_by_customer_160019（场景 contracts_create_and_filter_by_customer）—— 由 cases/contracts_create_and_filter_by_customer/ai_contracts_create_and_filter_by_customer_160019.json 自动生成。

**不要手改本文件**：改用例后重跑 generate 会覆盖它。要改行为 → 改用例 json / 场景 yml。
数据抽离在 scripts/datasets/ai_contracts_create_and_filter_by_customer_160019.json（脚本与数据分离）。
一个用例一个文件（P20）：便于按 id 管理、按变化增量重生成。

运行（推荐走 cli：自带资源预检 + run-id 隔离的日志/报告）:
  python -m framework.cli run --workers 2                 # 全量
  python -m framework.cli run --debug --case ai_contracts_create_and_filter_by_customer_160019        # 只调试这一条
裸跑 pytest（未设 HYBRID_RUN_ID 时日志落 log/latest/）:
  pytest scripts/generated/contracts_create_and_filter_by_customer/ai_contracts_create_and_filter_by_customer_160019.py -v
"""
