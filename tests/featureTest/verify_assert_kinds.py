...
为什么单独一个脚本、不叫 test_*.py：
  ...
  正向的、不需要浏览器的部分在 tests/frameworkTest/test_assert_kinds_render.py，那个随主套件跑。

跑法（需要 demo 在 8000 上：python -m demo.app）：
    cd ~/hybrid_gui_qa && source .venv/bin/activate
    python tests/featureTest/verify_assert_kinds.py          # 期望最后一行：全部符合预期 ✅
...
NEGATIVE = {
    ...
    "hidden": dict(steps=[CONTRACTS, _fill1005(), _search()],
                   asserts=[{"kind": "hidden", "selector": "#btn-search",
                             "desc": "搜索按钮实际可见"}]),
    "enabled": dict(steps=[CONTRACTS, _fill1005(), _search()],
                    asserts=[{"kind": "enabled", "selector": "#btn-export",
                              "desc": "导出按钮实际是 disabled"}]),
    "disabled": dict(steps=[CONTRACTS, _fill1005(), _search()],
                     asserts=[{"kind": "disabled", "selector": "#btn-search",
                               "desc": "搜索按钮实际可用"}]),
