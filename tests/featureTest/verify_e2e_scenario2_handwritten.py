...
耗时：约 1~3 分钟（取决于手搓用例数）。需要 demo 在跑（`tests/frameworkTest/demo_freshness.py --ensure`）。
"""
...
sys.path.insert(0, str(REPO / "tests" / "frameworkTest"))       # 共享辅助（demo_freshness / artifacts）
...
def main() -> int:
    ...
    r = run([PY, "tests/frameworkTest/demo_freshness.py", "--ensure", "--start-if-missing"], timeout=300)
