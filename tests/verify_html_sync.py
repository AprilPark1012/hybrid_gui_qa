...
退出码：0 通过 / 1 不同步（需 `python build_html.py` 后提交）/ 2 用法 / 3 跳过（缺文件）
"""
...
def _build(dst: Path, seed: str):
    ...
    r = subprocess.run([PY, "build_html.py"], cwd=str(BASE), env=env,
...
def main() -> int:
    ...
    print(f"  仓库: {BASE}")
    print("=" * 64)
    if not (BASE / "build_html.py").exists():
        print("  ⏭️ 跳过：找不到 build_html.py")
        return SKIP
    ...
    if not OUT.exists():
        ...
        print(f"  {'✅' if synced else '❌'} ② 同步：生成结果与 docs/training.html "
              f"{'逐字节一致' if synced else '不一致 ⇒ 需 python build_html.py 后提交'}")
        if not synced:
