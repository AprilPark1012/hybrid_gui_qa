...
`demo/` 的订单两页 + todo 页、`tests/` 的两个入口（`run_verifications.py` / `run_acceptance.py`）
...
跑法（秒级，不需要 demo/key/git）：
    python -m pytest tests/test_docs_sync.py -v
"""
...
def test_negative_missing_path_is_detected():
    assert missing_paths("见 <code>tests/definitely_missing_xyz.py</code>") == \
