...
跑法（不需要 demo、不需要浏览器、秒级）：
    cd ~/hybrid_gui_qa && source .venv/bin/activate && python -m pytest tests/frameworkTest/test_cli_exit_codes.py -q
"""
...
def test_ensure_dirs_only_declares_used_dirs():
    ...
    corpus = ""
    files = (list((repo / "framework").rglob("*.py"))
             + list((repo / "tests" / "frameworkTest").glob("*.py"))
             + list((repo / "tests" / "featureTest").glob("*.py"))
