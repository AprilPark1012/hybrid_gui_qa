...
跑法（秒级，不需要 demo / key / git）：
    python -m pytest tests/frameworkTest/test_readme_structure.py -v
"""
...
_CHECKED_PREFIXES = ("framework/", "tests/frameworkTest/", "tests/featureTest/", "docs/", "build_tools/", "cases/",
...
def test_negative_missing_path_is_detected():
    ...
    assert missing_paths("见 `tests/frameworkTest/definitely_missing_xyz.py` 与 `docs/oops.md`") == \
        ["docs/oops.md", "tests/frameworkTest/definitely_missing_xyz.py"]
    assert missing_paths("见 `README.md` 与 `cli.py`（裸文件名不参与比对）") == []
    assert missing_paths("见 `tests/featureTest/run_acceptance.py`（真实存在 ⇒ 不许误伤）") == []
