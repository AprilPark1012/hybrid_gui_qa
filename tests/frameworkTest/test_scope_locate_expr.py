...
（`tests/featureTest/verify_scope_locate.py`，要真浏览器 + 零 testid 夹具页）。但那条路慢、且失败时
...
跑法（秒级）：
    cd ~/hybrid_gui_qa && .venv/bin/python -m pytest tests/frameworkTest/test_scope_locate_expr.py -q
"""
...
# （tests/frameworkTest/test_import_targets.py）报「import 目标不存在」——那是污染判据、掩盖真正的搬家漏改。
