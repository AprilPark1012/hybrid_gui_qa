...
跑法（秒级，不需要 demo、不需要 DeepSeek key）：
    ...
    python -m pytest tests/frameworkTest/test_utf8_io.py -v
...
def test_verify_scripts_call_force_stdio():
    """`tests/featureTest/verify_*.py` 都必须统一 UTF-8 —— 否则中文 Windows 控制台上中文会显示成乱码。
    ...
    """
    missing = [p.name for p in sorted((REPO / "tests" / "featureTest").glob("verify_*.py"))
...
# 2026-09-15 AprilPark1012 Windows 验收实测：`pytest tests/frameworkTest/ -q` = 97 passed + 3 skipped。
...
def test_gbk_guard_is_cross_platform_and_actually_wired_in():
    ...
    """
    src = (REPO / "tests" / "frameworkTest" / "test_utf8_io.py").read_text(encoding="utf-8")
