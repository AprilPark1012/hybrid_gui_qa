...
③ 生成物不存在 / 0 字节 ⇒ 报错必须是**人话 + 建议命令**，不是「缺接线」天书
   （9 条契约判据的读取口已统一到 `tests/frameworkTest/artifacts.py`）。
...
def test_file_list_uses_git_inside_repo():
    ...
    assert "tests/frameworkTest/repo_files.py" in files, "清单里没有本文件 ⇒ 未跟踪文件没被收进来（假绿形态）"
...
# `bash tests/featureTest/run_verifications.sh` 直接报「无法将"bash"项识别为 cmdlet」⇒
# 二类验证对团队里的 Windows 用户等于不存在，而本框架的现场恰恰是 Windows。
# 口径：**唯一实现 = tests/featureTest/run_verifications.py**；`.sh` 退化成转发包装（防漂移判据见 ⑤ 节）。

def _run_runner(args):
    import subprocess
    return subprocess.run([sys.executable, "tests/featureTest/run_verifications.py", *args],
...
def test_sh_entry_is_thin_wrapper_over_python():
    ...
    """
    sh = (REPO / "tests" / "featureTest" / "run_verifications.sh").read_text(encoding="utf-8")
