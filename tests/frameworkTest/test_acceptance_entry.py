...
本文件是那个入口（`tests/featureTest/run_acceptance.py`）的**契约锁**：
...
⚠️ **TDD 口径**：本文件在 `tests/featureTest/run_acceptance.py` / `verify_e2e_scenario3_replay.py` 落地
...
ENTRY = REPO / "tests" / "featureTest" / "run_acceptance.py"
SCEN1 = REPO / "tests" / "featureTest" / "verify_e2e_scenario3_replay.py"
...
def _run_entry(args: list[str]):
    import subprocess
    return subprocess.run([sys.executable, "tests/featureTest/run_acceptance.py", *args],
