...
**为什么要有它**：R7 的两条新要求都靠自觉就会漂 ——
  · **R7-a 目录分工**：框架自验证住 `tests/frameworkTest/`、特性自验证住 `tests/featureTest/`；
...
判据清单：
  ...
  ② `tests/frameworkTest/` 必须有 `test_*.py`，且**仓库里别处不许再散落** `test_*.py`
  ③ `tests/featureTest/` 必须有 `verify_*.py` + 两个入口（`run_verifications.py` / `run_acceptance.py`）
  ④ runner 的收录口径必须是**自动 glob** `tests/featureTest/verify_*.py`（不许手写清单 ⇒ 不许有死脚本）
  ⑤ 每个 `tests/frameworkTest/test_*.py` 至少 1 个 `def test_`（**不许留空壳占数**）
...
REPO = Path(__file__).resolve().parents[2]          # tests/frameworkTest/ ⇒ 上两级才是仓库根
TESTS_DIR = REPO / "tests"                          # 容器目录（保留 · 2026-09-24 他定）
FRAMEWORK_TESTS = TESTS_DIR / "frameworkTest"
...
def test_tests_dir_is_the_container():
    """`tests/` 是**容器**（他 2026-09-24 定）：两类验证分住它下面，别把容器本身删掉。

    ⚠️ 我 2026-09-23 曾自作主张「取消 tests/ 目录」——他纠正：保留 `tests/`，把
    `tests/frameworkTest/` 与 `tests/featureTest/` 放进去，结构更合理。这条判据钉死这个结构。
    """
    assert TESTS_DIR.is_dir(), f"缺容器目录 {TESTS_DIR}（tests/ 不该被取消）"
    assert FRAMEWORK_TESTS.is_dir(), f"缺 {FRAMEWORK_TESTS}"
    assert FEATURE_TESTS.is_dir(), f"缺 {FEATURE_TESTS}"
    # 容器里只该有两类验证目录（+ __pycache__ 这类运行期产物）
    stray = sorted(p.name for p in TESTS_DIR.iterdir()
                   if p.is_dir() and p.name != "__pycache__"
                   and p.name not in ("frameworkTest", "featureTest"))
    assert not stray, f"tests/ 下出现了第三类目录（结构漂了）：{stray}"


# ---------------- ② 框架自验证在自己家 ----------------

def test_framework_tests_live_in_frameworkTest():
    assert FRAMEWORK_TESTS.is_dir(), "缺 tests/frameworkTest/（框架自验证的家）"
    here = find_test_modules(FRAMEWORK_TESTS)
    assert here, "tests/frameworkTest/ 里没有任何 test_*.py"
    stray = [p for p in find_test_modules(REPO)
             if not p.startswith("tests/frameworkTest/") and not p.startswith(".venv/")]
    assert not stray, f"这些 test_*.py 散落在 tests/frameworkTest/ 之外（应归位）：{stray[:8]}"


# ---------------- ③ 特性自验证在自己家 ----------------

def test_feature_tests_live_in_featureTest():
    assert FEATURE_TESTS.is_dir(), "缺 tests/featureTest/（框架特性自验证的家）"
    verifies = find_verify_modules(FEATURE_TESTS)
    assert verifies, "tests/featureTest/ 里没有任何 verify_*.py"
    for entry in ("run_verifications.py", "run_acceptance.py"):
        assert (FEATURE_TESTS / entry).is_file(), f"缺入口 tests/featureTest/{entry}"
    stray = [p for p in find_verify_modules(REPO) if not p.startswith("tests/featureTest/")]
    assert not stray, f"这些 verify_*.py 散落在 tests/featureTest/ 之外（应归位）：{stray[:8]}"
...
def test_runner_auto_collects_every_verify_script():
    ...
    assert runner_uses_glob(text), "runner 的收录口径必须是对 tests/featureTest/verify_*.py 的 glob"
...
def test_no_empty_test_modules():
    assert FRAMEWORK_TESTS.is_dir(), "缺 tests/frameworkTest/"
...
def test_negative_find_modules_ignores_noise(tmp_path):
    ...
    assert found == ["tests/frameworkTest/test_ok.py"], f"噪声目录没被排除：{found}"
...
def test_negative_runner_glob_detection():
    assert runner_uses_glob('items = sorted(repo.glob("tests/featureTest/verify_*.py"))') is True
    assert runner_uses_glob('items = [Path("tests/featureTest/verify_a.py")]') is False, \
