...
AprilPark1012 2026-09-22 刷新的 R7 四条：
  ① 框架自测通过 · ② 特性自测通过 · ③ demo 新鲜度闸门通过 · ④ E2E（场景1 / 场景2）通过

执行顺序（**他拍板：便宜先跑 + fail fast；编号 ≠ 执行顺序**）：
  ...
  [3] e2e                 ④ 场景2 = `cli run` 全量 · 场景1 = 离线回放端到端（`--skip-ai` 可跳过场景1）
...
场景1 的**在线真跑**（真调 DeepSeek）不在本入口：它要 key + 外网且 LLM 不稳定，按拍板只作**发版前手工一次**，
结果写进发行说明（见 P15）。
"""
...
HELP_TEXT = {
    ...
    "e2e": "④ E2E：场景2 = cli run 全量；场景1 = verify_e2e_scenario1_offline.py",
...
def step_e2e(py: str, *, skip_ai: bool) -> tuple[str, int, str, list[tuple[str, int]]]:
    """[3] ④ E2E：场景2（cli run 全量）→ 场景1（离线回放端到端）。

    返回 (状态, 退出码, 输出, [(子项名, 退出码), ...])；`--skip-ai` 时场景1 记为跳过（仍如实列出）。
    """
    ...
    logs.append("· 场景2（cli run 全量）：exit %d\n%s" % (rc2, tail2))
    ...
    if skip_ai:
        ...
        logs.append("· 场景1（离线回放端到端）：exit %d\n%s" % (rc1, tail1))
...
def _print_plan(skip_ai: bool) -> None:
    print("=" * 66)
    ...
    for i, sid in enumerate(STEP_IDS):
        tag = ""
        if sid == "e2e":
            tag = "   ← 场景1 将被跳过（--skip-ai）" if skip_ai else "   ← 场景1：离线回放（日常口径）"
        print(f"  [{i}] {sid:<19} {HELP_TEXT[sid]}{tag}")
    if skip_ai:
        print("       ├ scenario2  cli run 全量")
        print("       └ scenario1  [skip] 跳过 ⇒ 本次验收**不算通过**（SKIP ≠ 通过）")
    else:
        print("       ├ scenario2  cli run 全量")
        print("       └ scenario1  verify_e2e_scenario1_offline.py（离线回放，不联网）")


def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(
        prog="run_acceptance.py",
        description="R7 验收统一入口：闸门自检 → ③ demo 新鲜度 → ① 框架自测 → ④ E2E → ② 特性自测。",
    )
    ...
    for sid in todo:
        ...
        if sid == "gate_selfcheck":
            ...
            st, rc, tail, subs = step_e2e(py, skip_ai=args.skip_ai)
        else:
